...
public class DBRequest {
    ...
    public <T> Optional<T> extractAsSingle(Class<T> clazz) {
        List<T> results = executeQuery(clazz, true);
    ...
    public Optional<Map> extractAsSingleMap() {
        List<Map> results = executeQuery(Map.class, true);
        return results.isEmpty() ? Optional.empty() : Optional.of(results.get(0));
    }
    ...
    private <T> List<T> mapResultSet(ResultSet resultSet, Class<T> clazz) throws SQLException {
        ...
        Map<String, String> columnToFieldMap = createColumnToFieldMap(clazz, metaData);

        while (resultSet.next()) {
            T result = mapRow(resultSet, metaData, columnCount, clazz, columnToFieldMap);
            if (result != null) {
                results.add(result);
            }
        }
